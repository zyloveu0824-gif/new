...
# def merge_fl(nets, omega=0.5, pruning_rate=0.2, quantization_bits=8):
#     def prune_weights(weights, rate):
#         """
#         对权重进行修剪，保留前rate比例的权重。
#         """
#         flattened_weights = np.abs(weights.flatten())
#         threshold = np.percentile(flattened_weights, 100 * (1 - rate))
...
#         quantized_weights = np.round(weights / scale * (2**(bits - 1) - 1))
#         quantized_weights = quantized_weights / (2**(bits - 1) - 1) * scale
...
#             # 修剪当前和其他UAV模型的参数
#             pruned_target_params = prune_weights(target_params[i], pruning_rate)
#             pruned_other_params = [prune_weights(w[i], pruning_rate) for w in other_params]

#             # 其他UAV（边缘智能体）的参数 PS：求均值
#             others = np.sum(pruned_other_params, axis=0) / len(pruned_other_params)
            
#             # 量化当前和其他UAV模型的参数
#             quantized_target_params = quantize_weights(pruned_target_params, quantization_bits)
#             quantized_others = quantize_weights(others, quantization_bits)
            
#             # 当前UAV保留以权值（系数）为omega的参数，并混合其他UAV的参数
#             target_params[i] = omega * quantized_target_params + quantized_others * (1 - omega)

#         nets[agent_no].set_weights(target_params)  # 将新参数装载入模型
# def merge_fl(nets, learning_rate=0.01):
#     """
...
# def merge_fl(nets, num_samples=10):

#     # 获取所有模型的参数
#     all_params = [net.get_weights() for net in nets]
...
#         sampled_params = []
#         for layer in range(len(all_params[0])):
#             layer_samples = [params[layer] for params in all_params]
#             sampled_layer = random.choice(layer_samples)
#             sampled_params.append(sampled_layer)
#         sampled_params_list.append(sampled_params)

#     # 计算采样参数的均值，作为全局模型的参数
#     global_params = [np.mean([sample[layer] for sample in sampled_params_list], axis=0) for layer in range(len(all_params[0]))]

#     # 将全局参数分配给每个UAV模型
#     for net in nets:
#         net.set_weights(global_params)

# def merge_fl(nets, omega=0.5, p=1, eta=0.01, local_data_proportion=0.05):
#     # Θt−1 - Global model before update
#     # nets - List of all local models
#     global_model_weights = [np.mean([net.get_weights()[i] for net in nets], axis=0) for i in range(len(nets[0].get_weights()))]

#     # Iterating over each agent's local model
#     for agent_idx, net in enumerate(nets):
#         local_model_weights = net.get_weights()  # Θt−1i - Local model weights before update
#         updated_weights = []

#         # Applying ALA for top 'p' layers and direct averaging for the rest
#         for layer_idx, (local_layer, global_layer) in enumerate(zip(local_model_weights, global_model_weights)):
#             if layer_idx < p:
#                 # Adaptive Local Aggregation on higher layers
#                 # Wpi update step: simulate the adaptation using a fraction of local data
#                 gradient = eta * (global_layer - local_layer)  # Simplified gradient estimation
#                 update_factor = np.clip(1 - gradient, 0, 1)  # Simulating weight adjustment

#                 # Θ̂ti update: Adaptive aggregation of global and local modelsSSSSS
#                 adaptive_layer = local_layer + (global_layer - local_layer) * update_factor
#                 updated_weights.append(omega * local_layer + (1 - omega) * adaptive_layer)
#             else:
#                 # Simple average for lower layers
#                 updated_weights.append(omega * local_layer + (1 - omega) * global_layer)

#         # Update the local model weights with the newly computed weights
#         net.set_weights(updated_weights)
# def merge_fl(nets, global_control, local_controls, omega=0.5):
#     """
#     Merge function using SCAFFOLD algorithm.

#     Parameters:
#     - nets: list of network models for each agent.
#     - global_control: the global control variate (c). If None, it will be initialized.
#     - local_controls: list of local control variates (c_i) for each agent. If None, they will be initialized.
#     - omega: weight parameter for merging (0 <= omega <= 1).

#     Returns:
#     - global_control: updated global control variate.
#     - local_controls: updated list of local control variates.
#     """
#     # 初始化控制变量
#     if global_control is None:
#         global_control = [np.zeros_like(w) for w in nets[0].get_weights()]
#     if local_controls is None:
#         local_controls = [[np.zeros_like(w) for w in net.get_weights()] for net in nets]
...
#         # 对当前UAV模型中的“所有层的参数”进行循环
#         for i in range(len(target_params)):
#             others = np.sum([w[i] for w in other_params], axis=0) / len(other_params)  # 其他UAV（边缘智能体）的参数均值
#             # SCAFFOLD更新规则：结合全局控制和本地控制变量校正模型参数
#             correction = global_control[i] - local_controls[agent_no][i]
#             target_params[i] = omega * (target_params[i] + correction) + (1 - omega) * others

#         nets[agent_no].set_weights(target_params)  # 将新参数装载入模型

#     return global_control, local_controls
...
#             current_params[i] = current_params[i] - learning_rate * (current_params[i] - aggregated_params[i])
#         nets[agent_no].set_weights(current_params)
# def simulated_annealing_update(local_param, global_param, T):
#     # 使用模拟退火的规则更新参数
#     return local_param - T * (local_param - global_param)

# def merge_fl(nets, initial_temperature=1.0, cooling_rate=0.99):
#     # 获取每个网络的参数
#     net_params = [net.get_weights() for net in nets]

#     # 初始化全局参数
#     global_params = [np.zeros_like(param) for param in net_params[0]]
    
#     # 计算每一层的全局平均参数
#     for layer_idx in range(len(global_params)):
#         global_params[layer_idx] = np.mean([params[layer_idx] for params in net_params], axis=0)
...
#     for net in nets:
#         updated_params = []
#         local_params = net.get_weights()
#         for layer_idx in range(len(local_params)):
#             # 使用模拟退火更新参数
#             updated_param = simulated_annealing_update(local_params[layer_idx], global_params[layer_idx], T)
#             updated_params.append(updated_param)
#         net.set_weights(updated_params)
        
#         # 降低温度
#         T *= cooling_rate

# def compute_loss(global_params, local_params):
#     loss = 0
#     for g_param, l_param in zip(global_params, local_params):
#         loss += np.sum((g_param - l_param) ** 2)
#     return loss
...
#     # 计算每个客户端的损失
#     losses = np.array([compute_loss(global_params, params) for params in net_params])

#     # 计算Boosting权重
#     weights = compute_weights(losses)

#     # 使用Boosting权重进行加权聚合
#     for layer_idx in range(len(global_params)):
#         global_params[layer_idx] = np.sum([weights[net_idx] * net_params[net_idx][layer_idx] for net_idx in range(len(nets))], axis=0)
...
#         for layer_idx in range(len(local_params)):
#             # 使用FedBoost更新规则
#             updated_param = local_params[layer_idx] - learning_rate * (local_params[layer_idx] - global_params[layer_idx])
#             updated_params.append(updated_param)
#         net.set_weights(updated_params)
...
class MAACAgent(object):
    def __init__(self, env, tau, gamma, lr_sa, lr_sc, lr_aa, lr_ac, lr_ca, lr_cc, batch, epsilon=0.2):
        ...
        self.global_control_ua = None
        self.local_controls_ua=None
        self.global_control_uc = None
        self.local_controls_uc=None
        self.global_control_sa = None
        self.local_controls_sa=None
        self.global_control_ss = None
        self.local_controls_ss=None
    ...
    def train(self, max_epochs=2000, max_step=500, up_freq=8, render=False, render_freq=1, FL=False, FL_omega=0.5, anomaly_edge=False):
        ...
        while epoch < max_epochs:
            ...
            if epoch % up_freq == 1:
                ...
                # ①联邦更新
                if FL :
                    merge_fl(self.sensor_actors)  # 对所有sensor的actor网络进行联邦更新
                    merge_fl(self.sensor_critics)  # 对所有sensor的critic网络进行联邦更新
                    merge_fl(self.agent_actors)  # 对所有UAV的actor网络进行联邦更新
                    merge_fl(self.agent_critics)  # 对所有UAV的critic网络进行联邦更新
                # 对所有sensor进行循环
                for i in range(self.sensor_num):
