...
def collecting_rate(sensor, agent, sensors):
    # sensor和UAV之间的欧氏距离
    d = np.linalg.norm(np.array(sensor.position) - np.array(agent.position))
    # 根据公式计算收集速率
    Pl = 1 / (1 + a * np.exp(-b * (np.arctan(agent.h / d) - a)))
    ...
    # 计算总干扰，包括其他传感器对当前传感器的干扰
    I_total = 0
    for other_sensor in sensors:
        if other_sensor is not sensor:
            d_other = np.linalg.norm(np.array(other_sensor.position) - np.array(agent.position))
            Pl_other = 1 / (1 + a * np.exp(-b * (np.arctan(agent.h / d_other) - a)))
            L_other = Pl_other * yita0 + yita1 * (1 - Pl_other)
            # 累加干扰功率——对于每个其他传感器，它们的传输功率 other_sensor.ptr_tr 除以其路径损耗 L_other 和无人机噪声功率的平方 agent.noise_power**2，
            # 得到的值就是该传感器对当前传感器造成的干扰功率。所有这些干扰功率累加起来，就得到了总干扰功率 I_total。
            I_total += other_sensor.ptr_tr / (L_other * agent.noise_power**2)
    
    # 计算SINR
    SINR = agent.ptr_col / (L * sensor.noise_power**2 + I_total)
    
    # 使用SINR计算收集速率
    rate = sensor.bandwidth * np.log2(1 + SINR)
...
class MEC_world(object):
    def __init__(self, map_size, agent_num, sensor_num, obs_r, speed, collect_r, MAX_EP_STEPS, max_size=1, sensor_lam=0.5):
        ...
        # 初始化传感器的位置，速度和方向
        # 为了避免边界，我们调整了随机数生成的范围
        self.sensor_num = sensor_num
        self.sensor_num = sensor_num
        self.map_size = map_size
        # self.grid_interval = grid_interval
        self.grid_size = self.map_size // self.grid_interval
        self.sensor_pos = [[0]*sensor_num for _ in range(2)]
        self.sensor_direction = [0]*sensor_num
        self.sensor_targets = [[0]*sensor_num for _ in range(2)]
        # self.sensor_pos = [
        #     [random.randint(1, (self.map_size // self.grid_interval) - 2) * self.grid_interval for _ in range(sensor_num)],
        #     [random.randint(1, (self.map_size // self.grid_interval) - 2) * self.grid_interval for _ in range(sensor_num)]
        # ]
        # self.generate_new_target(i)
        for i in range(self.sensor_num):
            initial_x = random.randint(0, self.grid_size - 1) * self.grid_interval
            initial_y = random.randint(0, self.grid_size - 1) * self.grid_interval
            self.sensor_pos[0][i], self.sensor_pos[1][i] = initial_x, initial_y
            self.generate_new_target(i)
            print(f"Sensor {i} initialized at {self.sensor_pos[0][i]}, {self.sensor_pos[1][i]} with target {self.sensor_targets[0][i]}, {self.sensor_targets[1][i]}")
    ...
    def generate_new_target(self, sensor_index):
        self.sensor_targets[0][sensor_index] = random.randint(0, self.grid_size - 1) * self.grid_interval
        self.sensor_targets[1][sensor_index] = random.randint(0, self.grid_size - 1) * self.grid_interval
        print(f"New target for sensor {sensor_index} is {self.sensor_targets[0][sensor_index]}, {self.sensor_targets[1][sensor_index]}")

    def move_sensor(self, sensor_index):
        current_x, current_y = self.sensor_pos[0][sensor_index], self.sensor_pos[1][sensor_index]
        target_x, target_y = self.sensor_targets[0][sensor_index], self.sensor_targets[1][sensor_index]
        print(f"Sensor {sensor_index} moving from {current_x}, {current_y} to {target_x}, {target_y}")

        if current_x == target_x and current_y == target_y:
            self.generate_new_target(sensor_index)
            return

        # Calculate movement
        if current_x % self.grid_interval == 0 and current_y % self.grid_interval == 0:
            self.choose_direction(sensor_index, target_x, target_y)

        speed = 5  # Set speed
        dx, dy = self.get_movement_delta(self.sensor_direction[sensor_index], speed)
        new_x = current_x + dx
        new_y = current_y + dy

        # Update position
        self.sensor_pos[0][sensor_index] = new_x
        self.sensor_pos[1][sensor_index] = new_y
        print(f"Sensor {sensor_index} moved to {new_x}, {new_y}")

    def choose_direction(self, sensor_index, target_x, target_y):
        current_x, current_y = self.sensor_pos[0][sensor_index], self.sensor_pos[1][sensor_index]
        if current_x != target_x:
            self.sensor_direction[sensor_index] = 0 if target_x > current_x else math.pi
        elif current_y != target_y:
            self.sensor_direction[sensor_index] = math.pi / 2 if target_y > current_y else 3 * math.pi / 2
        print(f"Sensor {sensor_index} direction set to {self.sensor_direction[sensor_index]}")

    def get_movement_delta(self, direction, speed):
        if direction in [0, math.pi]:  # Horizontal movement
            dx = speed if direction == 0 else -speed
            return dx, 0
        elif direction in [math.pi / 2, 3 * math.pi / 2]:  # Vertical movement
            dy = speed if direction == math.pi / 2 else -speed
            return 0, dy
        return 0, 0  # Default case, should not happen
    # def move_sensor(self, sensor_index):
    #     if 0 <= sensor_index < self.sensor_num:
