...
class MAACAgent(object):
    ...
    def replay(self):
        ...
        for no, sensor_memory in self.sensor_memory.items():
            ...
            # samples = random.sample(sensor_memory[-self.batch_size * 2:], int(self.batch_size))
            # samples = random.sample(sensor_memory[-self.batch_size * 2:], int(self.batch_size))
        ...
        for no, agent_memory in self.agent_memory.items():
            ...
            # 对经验缓冲区内的数据进行采样
            # 采样规则：最新的0.25*batch_size条经验数据+最新的2*batch_size条经验数据中随机抽取的0.75*batch_size条
            samples = random.sample(agent_memory[-self.batch_size * 2:], int(self.batch_size ))
        ...
        center_samples = random.sample(self.center_memory[-self.batch_size * 2:], int(self.batch_size ))
