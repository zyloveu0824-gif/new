...
def sensor_actor(input_dim_list):
    # print("shape of input_dim_list:",input_dim_list.shape)
    # Input
    ...
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    # print("shape of sensor_buffer_state:",sensor_buffer_state.shape)
    # print(sensor_buffer_state)
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    ...
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    # print("shape of computing_dev:",computing_dev.shape)
    # print(computing_dev)
    # print("----------------------------------------------")
    # print("----------------------------------------------")
    # print("----------------------------------------------")
...
def get_log_probs(actor, sensor_data_state, actions):
    ...
    # 创建Bernoulli分布对象
    dist = tfp.distributions.Normal(loc=logits, scale=1.0)  # 假设标准差为1
    ...
    return log_probs
def get_log_probs_uav(model, inputs, actions):
    action_output = model(inputs)
    move_out_pred = action_output[0][0]
    # 预测模型输出
    op_dist_pred = action_output[1][0]
    # 对 move_out_pred 进行扁平化处理
    move_out_pred_flat = tf.reshape(move_out_pred, [tf.shape(move_out_pred)[0], -1])

    # 创建 move_dist 和 op_dist 的概率分布
    move_dist = tfp.distributions.Categorical(logits=move_out_pred_flat)
    op_dist = tfp.distributions.Categorical(logits=op_dist_pred)

    # 计算 move_dist 的对数概率
    # 由于 move 是一个多维空间的动作，我们需要将它转换为一个扁平索引
    move_indices = tf.argmax(actions[0], axis=-1)  # 假设 actions[0] 已经是扁平化的索引
    log_prob_move = move_dist.log_prob(move_indices)
    # print("-----------------move-------------------")
    # print(log_prob_move)
    # 计算 op_dist 的对数概率
    op_indices = tf.argmax(actions[1], axis=-1)  # 假设 actions[1] 已经是适当的索引
    log_prob_op = op_dist.log_prob(op_indices)
    # print("-----------------op-------------------")
    # print(log_prob_op)
    # 总对数概率
    total_log_prob = tf.reduce_sum(log_prob_move) + tf.reduce_sum(log_prob_op)
...
#     # 预测模型输出
#     op_dist_pred = action_output[1][0]
#     # move_out_pred 已经是概率，将其重新整形为合适的分类分布输入
#     move_out_pred_flat = tf.reshape(move_out_pred, [tf.shape(move_out_pred)[0], -1])  # 扁平化处理
#     move_dist = tfp.distributions.Categorical(probs=move_out_pred_flat)
#     log_prob_move = move_dist.log_prob(tf.argmax(actions[0], axis=-1))  # 假设actions[0]已经是选择的位置的独热编码

#     # op_dist_pred 是离散动作的概率输出，使用 Categorical 分布建模
#     op_dist = tfp.distributions.Categorical(logits=op_dist_pred)
#     log_prob_op = op_dist.log_prob(tf.argmax(actions[1], axis=-1))

#     # 综合对数概率
#     total_log_prob = log_prob_move + log_prob_op
#     return total_log_prob
...
class MAACAgent(object):
    def __init__(self, env, tau, gamma, lr_sa, lr_sc, lr_aa, lr_ac, lr_ca, lr_cc, batch, epsilon=0.2):
        ...
        for i in range(self.env.sensor_num):
            ...
            new_sensor_actor = sensor_actor([self.s_buffstate_shape])
            target_sensor_actor = sensor_actor([self.s_buffstate_shape])
            # 更新target权重参数
        ...
        for i in range(self.env.agent_num):
            ...
            self.agent_old_actors.append(new_agent_actor)
    ...
    def replay(self):
        ...
        for no, sensor_memory in self.sensor_memory.items():
            ...
            # 提取并整理各类数据
            sensor_data_state = np.vstack([sample[0][0] for sample in samples])
            sensor_computing = np.vstack([sample[1][0] for sample in samples])
            s_reward = tf.expand_dims([sample[2] for sample in samples], axis=-1)
            new_sensor_data_state = np.vstack([sample[3][0] for sample in samples])
            ...
            # 确保s_reward和sq_future的数据类型一致
            s_reward = tf.cast(s_reward, dtype=tf.float32)
            sq_future = tf.cast(sq_future, dtype=tf.float32)

            s_target_qs = s_reward + self.gamma * sq_future

            # PPO: 计算优势函数
            with tf.GradientTape(persistent=True) as tape:
                current_values = self.sensor_critics[no]([sensor_data_state, sensor_computing])
                advantages = s_target_qs - current_values
                advantages = (advantages - tf.reduce_mean(advantages)) / (tf.math.reduce_std(advantages) + 1e-8)

            # 1. 训练critic网络 train critic
            with tf.GradientTape() as tape:
                sq_values = self.sensor_critics[no]([sensor_data_state, sensor_computing])
                critic_loss = tf.reduce_mean(tf.math.square(sq_values - tf.cast(s_target_qs, dtype=tf.float32)))
                ...
                sc_loss = critic_loss + regularization_loss
            critic_grad = tape.gradient(sc_loss, self.sensor_critics[no].trainable_variables)
            self.sensor_critic_old_gard[no] = [grad.numpy() for grad in critic_grad]
            self.sensor_critic_old_weights[no] = [var.numpy() for var in self.sensor_critics[no].trainable_variables]
            self.sensor_critic_opt[no].apply_gradients(zip(critic_grad, self.sensor_critics[no].trainable_variables))
            self.sensor_critic_gard[no] = [grad.numpy() for grad in critic_grad]
            self.sensor_critic_weights[no] = [var.numpy() for var in self.sensor_critics[no].trainable_variables]

            # 2. 训练actor网络 train actor using PPO
            with tf.GradientTape() as tape:
                s_actions = self.sensor_actors[no]([sensor_data_state])
                old_log_probs = get_log_probs(self.sensor_actors[no], sensor_data_state, s_actions)
                
                new_log_probs = get_log_probs(self.sensor_actors[no], new_sensor_data_state, new_sensor_action)
                ...
                regularization_loss = -mu * regularization_term
                sa_loss = policy_loss + regularization_loss

            sa_grad = tape.gradient(sa_loss, self.sensor_actors[no].trainable_variables)
            ...
            self.sensor_actor_opt[no].apply_gradients(zip(sa_grad, self.sensor_actors[no].trainable_variables))
        ...
        for no, agent_memory in self.agent_memory.items():
            ...
            # 根据（env.step更新后）下一时隙的状态预测下一时隙动作和reward（这个reward是利用target_agent_critic网络得到的）
            new_actions = self.target_agent_actors[no].predict([new_state_map, new_total_data_state, new_done_data_state, new_band])
            q_future = self.target_agent_critics[no].predict([new_state_map, new_total_data_state, new_done_data_state, new_actions[0], new_actions[1], new_band])
            q_future = tf.cast(q_future,dtype=tf.float32)
            a_reward = tf.cast(a_reward,dtype=tf.float32)
            target_qs = a_reward + q_future * self.gamma
	
            # 计算优势函数
            with tf.GradientTape(persistent=True) as tape:
                current_values = self.agent_critics[no]([state_map, total_data_state, done_data_state, move, op_softmax, band])
                advantages = target_qs - current_values
                advantages = (advantages - tf.reduce_mean(advantages)) / (tf.math.reduce_std(advantages) + 1e-8)
            ...
            # 1.3 训练actor网络 train actor using PPO
            with tf.GradientTape() as tape:
                move_dist = new_actions[0][0]
                op_dist = new_actions[1][0]  # agent_actor输出中的收集、卸载
                old_log_probs = get_log_probs_uav(self.agent_old_actors[no], [state_map, total_data_state, done_data_state, band], [move, op_softmax])
                new_log_probs = get_log_probs_uav(self.agent_actors[no], [state_map, total_data_state, done_data_state, band], [move, op_softmax])
                # 计算概率比
                ratio = tf.exp(new_log_probs - old_log_probs)
                # 计算裁剪后的PPO目标函数
                surr1 = ratio * advantages
                ...
                regularization_term = tf.add_n([tf.nn.l2_loss(current_weight - old_weight)
                                                for current_weight, old_weight in zip(self.agent_actor_weights[no], self.agent_actor_old_weights[no])])
                regularization_loss = mu * regularization_term
                aa_loss = ppo_loss + regularization_loss

            self.agent_old_actors[no].set_weights(self.agent_actors[no].get_weights())
            aa_grad = tape.gradient(aa_loss, self.agent_actors[no].trainable_variables)
            self.agent_actor_old_gard[no] = [grad.numpy() for grad in aa_grad]
            self.agent_actor_old_weights[no] = [var.numpy() for var in self.agent_actors[no].trainable_variables]
            self.agent_actor_opt[no].apply_gradients(zip(aa_grad, self.agent_actors[no].trainable_variables))
        ...
        # # 对经验缓冲区内的数据进行采样
        # # 采样规则：最新的0.25*batch_size条经验数据+最新的2*batch_size条经验数据中随机抽取0.75*batch_size条
        # center_samples = self.center_memory[-int(self.batch_size * self.sample_prop):] + random.sample(self.center_memory[-self.batch_size * 2:], int(self.batch_size * (1 - self.sample_prop)))
        ...
        # # 根据（env.step更新后）下一时隙的状态预测下一时隙动作和reward（这个reward是利用target_center_critic网络得到的）
        # new_c_actions = self.target_center_actor.predict([new_done_buffer_list, new_pos_list])
        # cq_future = self.target_center_critic.predict([new_done_buffer_list, new_pos_list, new_c_actions])
        # c_target_qs = c_reward + cq_future * self.gamma
        ...
        # # 计算优势函数
        # with tf.GradientTape(persistent=True) as tape:
        #     current_values = self.center_critic([done_buffer_list, pos_list, bandvec_act])
        #     advantages = c_target_qs - current_values

        # # 2.2 训练center critic网络 train center critic
        # with tf.GradientTape() as tape:
        #     cq_values = self.center_critic([done_buffer_list, pos_list, bandvec_act])
        #     lcc_loss = tf.reduce_mean(tf.math.square(cq_values - tf.cast(c_target_qs, dtype=tf.float32)))
        ...
        # # 2.3 训练center actor网络 train center actor using PPO
        # with tf.GradientTape() as tape:
        #     old_log_probs = self.get_log_probs(self.center_actor, done_buffer_list, pos_list, bandvec_act)
        #     c_act = self.center_actor([done_buffer_list, pos_list])
        #     new_log_probs = self.get_log_probs(self.center_actor, done_buffer_list, pos_list, c_act)
        ...
        #                                     for current_weight, old_weight in zip(self.center_actor.trainable_variables, self.center_actor_old_weights)])
        #     regularization_loss = mu * regularization_term
        #     total_ca_loss = ppo_loss + regularization_loss

        # ca_grad = tape.gradient(total_ca_loss, self.center_actor.trainable_variables)
        # self.center_actor_old_gard = [grad.numpy() for grad in ca_grad]
        # self.center_actor_old_weights = [var.numpy() for var in self.center_actor.trainable_variables]
        # self.center_actor_opt.apply_gradients(zip(ca_grad, self.center_actor.trainable_variables))  # 自动更新模型参数
        # self.center_actor_gard = [grad.numpy() for grad in ca_grad]
        ...
        # # 总结center的actor和critic网络的训练平均误差信息 summary info
        # self.summaries['center-critic_loss'] = cc_loss
        # self.summaries['center-actor_loss'] = total_ca_loss
